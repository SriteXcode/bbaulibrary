...
export default function MyIssues() {
    ...
    return (
        <div className="container mx-auto p-8">
            <h2 className="text-3xl font-extrabold text-gray-900 mb-6 border-b-2 border-red-600 pb-2">
                My Books & Requests
            </h2>

            {message && (
                <div className="p-3 mb-4 text-sm text-red-700 rounded-lg flex mt-16 justify-center items-center space-x-3">
                    {loading && (
                        <div className="border-12 w-12 h-12 border-2 border-red-400 border-t-transparent rounded-full animate-spin"></div>
                    )}
                    <span>{message}</span>
                </div>
            )}
                    ...
                    issues.map(issue => (
                        <div key={issue._id} className="bg-white shadow-md rounded-lg p-5 flex justify-between items-center border-l-4 border-red-600">
                            <div>
                                ...
                                {issue.fine > 0 && (
                                    <p className="text-md font-semibold text-red-600 mt-1">
                                        Fine Due: ₹{issue.fine.toFixed(2)}
                                    </p>
                                )}

                                {issue.status === 'returned' && (
                                    <p className="text-sm text-green-600">
                                        Paid: ₹{issue.paidAmount}
                                    </p>
                                )}
                            </div>
